...
function SAXParser() {
	...
	var attr_string = ''; // string attributes
	var attr_posstart = 0; // 
	var attr_res; // caching result parsing attributes , null - analysis was not performed, object - hash attributes, true - no attributes, false - invalid xml

	/*
		parses the attributes as required. important! - function does not generate an exception.
		if there was a parse error comes back false
		if there are no attributes and parsing is successful then comes back true
		if there are attributes that comes back object ( hash)
	*/
	...
	function getAttrs() {
		...
		*/
		...
		for(; i < l; i++) {
			...
			if (w < 65 || w >122 || (w>90 && w<97) ) { // expect symbol
				//console.log('error attr 1')
				return attr_res = false; // error. invalid char
			};

			for(j = i + 1; j < l; j++) { // check all the characters of the name attribute
				w = s.charCodeAt(j);
			...
			i = j + 1; // seq. semvol already verified because you need to check sleduyushy

			if (isNamespace) { // 
				if (hasSurmiseNS) {
					// there is a suspicion that attribute is present xmlns
		...
		if (!ok) {
			return attr_res = true;  // attribute is not present, there is no error, too
		};
	...
	function parse(xml) {
		...
		, xmlns
		, stopIndex = 0
		, stop // used in the analysis "namespace" . If you meet an unknown space that events are not generated
		, _nsmatrix
		, ok
		;
		...
		while(j !== -1) {
			...
			if (w === 47) { // </...
				...
				// check that should be closed totzhe tag that opened
				...
				// check that there is no excess zakryvayushem tag
				for(; q < j; q++) {
					w = xml.charCodeAt(q);
				...
				for(q = 1, y = x.length; q < y; q++) {
					...
					if (w===32 || (w<14 && w > 8)) { // \f\n\r\t\v space
						...
						attr_res = null; // maybe there atiributy
						break;
					};
			...
			if (isNamespace) {
				...
				if (!xmlns) {
					if (tagend) {
						...
						stopIndex = 1; // the first element that does not specify a namespace
						attr_res = true;
					};
			...
			//string_node = xml.substring(i, j+1); // text nodes as is
