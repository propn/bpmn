...
function EasySAXParser() {
	...
	var onTextNode = nullFunc, onStartNode = nullFunc, onEndNode = nullFunc, onCDATA = nullFunc, onError = nullFunc;
	var onComment, onQuestion, onAttention, is_onComment, is_onQuestion, is_onAttention;
	...
	function parse(xml) {
		...
		while(j !== -1) {
			...
			if (tagstart) { // is_onStartNode
				...
				ok = onStartNode(elem, getAttrs, unEntities, tagend, getStringNode);
			...
			if (tagend) {
				ok = onEndNode(elem, unEntities, tagstart, getStringNode);
